...
namespace Gaskellgames.EditorOnly
{
    ...
    public static class HierarchyUtility
    {
        ...
        private static void OnGameObjectDestroyed(EntityId entityID, GameObject parent, Scene scene)
        {
            if (!GaskellgamesSettings_SO.Instance) { return; }
            if (!GaskellgamesSettings_SO.Instance.ShowHierarchyIcons && !GaskellgamesSettings_SO.Instance.ShowHierarchyBreadcrumbs) { return; }
            if (!parent) { return; }
...
#else
            EntityId parentID = parent.GetInstanceID();
#endif
...
#else
            EntityId entityID = gameObject.GetInstanceID();
#endif
            if (hierarchyObjectCache.TryGetValue(entityID, out HierarchyData hierarchyData))
            {
...
#if UNITY_6000_4_OR_NEWER
        private static void DrawHierarchy(EntityId entityID, Rect position)
        {
            DrawHierarchyComponentIcons(entityID, position);
            DrawHierarchyBreadcrumbs(entityID, position);
        }
#else
        private static void DrawHierarchy(int entityID, Rect position)
        {
            DrawHierarchyComponentIcons(entityID, position);
            DrawHierarchyBreadcrumbs(entityID, position);
        }
...
#else
            EntityId parentID = transform.parent.gameObject.GetInstanceID();
#endif
        ...
        {
#if UNITY_6000_4_OR_NEWER
            EntityId entityID = parent.GetEntityId();
#else
            EntityId entityID = parent.GetInstanceID();
#endif
            if (hierarchyObjectCache.TryGetValue(entityID, out HierarchyData oldHierarchyData))
            {
        ...
        private static void CacheHierarchyObject(GameObject gameObject, int indentLevel, List<bool> parentIsFinalChild, bool isFinalChild)
        {
            ...
            hierarchyObjectCache.Remove(entityID);
            hierarchyObjectCache.TryAdd(entityID, hierarchyData);
        }
        ...
        private static void DrawHierarchyComponentIcons(EntityId entityID, Rect position)
        {
            if (!GaskellgamesSettings_SO.Instance) { return; }
            if (!GaskellgamesSettings_SO.Instance.ShowHierarchyIcons) { return; }
            if (hierarchyIcons == null) { return; }
            if (hierarchyObjectCache == null) { return; }
            if (!hierarchyObjectCache.TryGetValue(entityID, out HierarchyData hierarchyData)) { return; }
        ...
        /// <param name="entityID"></param>
        /// <param name="position"></param>
        private static void DrawHierarchyBreadcrumbs(EntityId entityID, Rect position)
        {
            if (!GaskellgamesSettings_SO.Instance) { return; }
