...
namespace Gaskellgames.FolderSystem.EditorOnly
{
    ...
    public class HierarchyFolderIcons
    {
        ...
        static HierarchyFolderIcons()
        {
            ...
            EditorApplication.hierarchyWindowItemByEntityIdOnGUI -= EditorApplication_hierarchyWindowItemOnGUI;
            EditorApplication.hierarchyWindowItemByEntityIdOnGUI += EditorApplication_hierarchyWindowItemOnGUI;
#else
        ...
        private static void EditorApplication_hierarchyWindowItemOnGUI(
#if UNITY_6000_4_OR_NEWER
            EntityId entityID,
#else
            int entityID,
...
#else
            GameObject gameObject = EditorUtility.InstanceIDToObject(entityID) as GameObject;
#endif
            if (gameObject != null)
            {
...
#else
                    bool isSelected = Selection.instanceIDs.Contains(entityID);
#endif
        ...
        private static bool IsExpanded(GameObject gameObject)
        {
            ...
            ulong[] expandedIDs = getExpandedIDs.Invoke(lastInteractedHierarchyWindow.GetValue(null), null) as ulong[];
            if (expandedIDs == null) { return false; }
            
            // check if referenced gameObject is expanded
#if UNITY_6000_4_OR_NEWER
            return expandedIDs.Contains(EntityId.ToULong(gameObject.GetEntityId()));
#else
            return expandedIDs.Contains((ulong)gameObject.GetInstanceID());
#endif
        }
